...
class RobertaMultiTaskClassifier(RobertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.attach_dropout = torch.nn.Dropout(dropout_prob)
        self.attach_classifier = torch.nn.Linear(config.hidden_size, 3)
        self.init_weights()
    ...
    def forward(self, x, numeral_position, task):
        ...
        if task == 'attach':
            ...
            hidden_states = outputs[0]
            x = torch.stack([hidden_states[idx, p, :] for idx, p in enumerate(numeral_position)])
            x = self.attach_dropout(x)
            x = self.attach_classifier(x)
            return x
