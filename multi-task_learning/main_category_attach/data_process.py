...
class AttachTaskDataset(Dataset):
...
def attach_task_collate_fn(batch):
...
def get_attach_task_train_dataloader(args, tokenizer):
	...
	train_dataset = AttachTaskDataset(inputs, numeral_position, sign_id)
	train_dataloader = DataLoader(
						train_dataset, 
						args['batch_size'], 
						shuffle=True, 
						collate_fn=attach_task_collate_fn, 
						num_workers=4, 
					)
	return train_dataloader

def get_attach_task_eval_dataloader(args, tokenizer):
	sign = get_all_sign(args)
	sign2id = {sign: i for i, sign in enumerate(sign)}
	with open(args['eval_data'], 'r') as fp:
		data = json.load(fp)
	...
	eval_dataset = AttachTaskDataset(inputs, numeral_position, sign_id)
	eval_dataloader = DataLoader(
						eval_dataset, 
						args['batch_size'], 
						shuffle=False, 
...
def get_attach_task_test_dataloader(args, tokenizer):
	sign = get_all_sign(args)
	sign2id = {sign: i for i, sign in enumerate(sign)}
	with open(args['test_data'], 'r') as fp:
		data = json.load(fp)
	...
	test_dataset = AttachTaskDataset(inputs, numeral_position, sign_id)
	test_dataloader = DataLoader(
						test_dataset, 
						args['batch_size'], 
						shuffle=False, 
