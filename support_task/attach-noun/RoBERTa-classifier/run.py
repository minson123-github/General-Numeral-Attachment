...
from data_process import get_train_dataloader, get_eval_dataloader, get_test_dataloader, get_all_sign
...
def train(args):
	...
	model = RobertaNumeralClassifier.from_pretrained(args['pretrained_model'], num_labels=len(sign))
...
def test(args):
	...
	model = RobertaNumeralClassifier.from_pretrained(args['testing_model'], num_labels=len(sign))
		...
		for batch in pbar:
			...
			for logit, label in zip(logits, sign_id):
				...
				if max_arg == label.item():
					n_correct += 1
				total += 1
				log.append((sign[max_arg], sign[label.item()]))
			acc = n_correct / total
			pbar.set_postfix({'acc': '{:.3f}%'.format(acc * 100), 'loss': '{:.3f}'.format(loss.item())})
