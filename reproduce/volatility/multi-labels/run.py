    ...
    for epoch in range(args['n_epoch']):
        ...
        with torch.no_grad():
            ...
            for batch_embeds, batch_labels in test_dataloader:
                outputs = transformer_layer(batch_embeds.cuda())
                outputs = linear_layer(outputs).cpu()
                r3_outputs = outputs[:, 0]
                r3_batch_labels = batch_labels[:, 0]
                loss = loss_fn(r3_batch_labels, r3_outputs)
                r3_total += loss.item()
                r3_size += 1

                r7_outputs = outputs[:, 1]
                r7_batch_labels = batch_labels[:, 1]
                loss = loss_fn(r7_batch_labels, r7_outputs)
                r7_total += loss.item()
                r7_size += 1

                r15_outputs = outputs[:, 2]
                r15_batch_labels = batch_labels[:, 2]
                loss = loss_fn(r15_batch_labels, r15_outputs)
                r15_total += loss.item()
                r15_size += 1

                r30_outputs = outputs[:, 3]
                r30_batch_labels = batch_labels[:, 3]
